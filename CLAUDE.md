...
1. **Validation**: Require correct flags (--kubeconfig, --context) and mode-correct wrapping
2. **Command classification**: Block write ops in read-only mode, gcloud writes always
...
| File | Purpose |
|------|---------|
| `rwenv/lib/rwenv-utils.sh` | Core utilities: config loading, write detection, env file generation |
| `rwenv/hooks/transform-commands.sh` | PreToolUse hook: validates, enforces safety, auto-approves |
...
| `.claude/rwenv-env` | Per-project runtime config (generated by /rwenv-set, sourced by hooks/agents) |
...
```bash
# Test hook validation and auto-approval
