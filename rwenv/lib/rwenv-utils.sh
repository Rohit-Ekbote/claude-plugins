...
#
# Usage: write_rwenv_env [directory] <rwenv_name>
# Args:
#   directory  - Target directory (defaults to $PWD)
#   rwenv_name - Name of rwenv to generate config for (required)
...
write_rwenv_env() {
    ...
    if [[ -z "$rwenv_name" ]]; then
        echo "ERROR: rwenv name parameter is required" >&2
        return 1
    fi
